...
class CoreWorker : public std::enable_shared_from_this<CoreWorker> {
 ...
 public:
  ...
  void HandleAssignObjectOwner(rpc::AssignObjectOwnerRequest request,
                               ...
                               rpc::SendReplyCallback send_reply_callback);
  // Handle incoming gossip future metadata from peer workers
  void HandleGossipFuture(rpc::GossipFutureRequest request,
 ...
 private:
  ...
  /// Fields related to gossip-based owner failure recovery.
  ///
  // Gossip table: maps object ID to task spec for recovery.
  // Protected by gossip_mu_.
  absl::Mutex gossip_mu_;
