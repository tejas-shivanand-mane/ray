...
RecoverySuccessionManager::RecoverySuccessionManager(
    rpc::Address self_address)
...
rpc::RecoveryManifest
RecoverySuccessionManager::BuildInitialManifest(
...
void RecoverySuccessionManager::RegisterOwnedTask(
  ...
  if (!task_proto.has_recovery_manifest() ||
      task_proto.task_id().empty()) {
    return;
  }

  const TaskID task_id =
      TaskID::FromBinary(task_proto.task_id());
  ...
  for (size_t return_index = 0;
       return_index < returned_refs->size();
       ++return_index) {
    rpc::ObjectReference &returned_ref =
        returned_refs->at(return_index);
    ...
    metadata.set_return_index(
        static_cast<uint32_t>(return_index));
    ...
    returned_ref.mutable_recovery_metadata()->CopyFrom(
        metadata);
  }
}

void RecoverySuccessionManager::RegisterExecutorTask(
    const rpc::TaskSpec &task_spec) {
  if (!IsEligibleTask(task_spec) ||
  ...
  const TaskID task_id =
      TaskID::FromBinary(task_spec.task_id());
...
void RecoverySuccessionManager::RegisterBorrowedObject(
  ...
  borrowed_state.task_id =
      TaskID::FromBinary(metadata.task_id());
  borrowed_state.return_index =
      metadata.return_index();
  borrowed_state.cached_manifest.CopyFrom(
      metadata.manifest());

  absl::MutexLock lock(&mutex_);

  borrowed_objects_[object_id] =
      std::move(borrowed_state);
  ...
  for (rpc::TaskArg &arg :
    ...
    for (rpc::ObjectReference &nested_ref :
      ...
      const ObjectID nested_id =
          ObjectID::FromBinary(
              nested_ref.object_id());

      const auto metadata_it =
          object_recovery_metadata_.find(nested_id);
...
bool RecoverySuccessionManager::TryRecoverObject(
    const ObjectID &object_id) {
