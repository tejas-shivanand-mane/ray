...
void CoreWorker::RegisterOwnershipInfoAndResolveFuture(
  ...
  if (object_status.has_object() && !reference_counter_->OwnedByUs(object_id)) {
    ...
    if (gossip_recovery_enabled_) {
      absl::MutexLock lock(&gossip_mu_);
      gossip_pending_resolve_.insert(object_id);
    }
...
Status CoreWorker::Put(const RayObject &object,
  ...
  *object_id = ObjectID::FromIndex(worker_context_->GetCurrentInternalTaskId(),
  ...
  // GOSSIP STORE: record ownership of this ray.put() object
  // so we can recover metadata if this worker dies
  if (gossip_recovery_enabled_) {
    absl::MutexLock lock(&gossip_mu_);
    // Store a sentinel entry — no task spec for ray.put()
    // but we record the object ID so recovery can find the value in Plasma
    gossip_table_[*object_id] = rpc::TaskSpec();
    RAY_LOG(INFO).WithField(*object_id)
        << "GOSSIP_STORE_PUT: stored ray.put object "
        << (*object_id).Hex()
        << " table_size=" << gossip_table_.size();
  }
...
Status CoreWorker::SealOwned(const ObjectID &object_id,
  ...
  if (status.ok()) {
    // GOSSIP STORE: record this ray.put() object for owner failure recovery
    if (gossip_recovery_enabled_) {
      absl::MutexLock lock(&gossip_mu_);
      gossip_table_[object_id] = rpc::TaskSpec();
      RAY_LOG(INFO).WithField(object_id)
...
std::vector<rpc::ObjectReference> CoreWorker::SubmitTask(
  ...
  // GOSSIP STORE: save task spec for each return object
  // so we can recover ownership if this worker dies
  if (gossip_recovery_enabled_) {
    absl::MutexLock lock(&gossip_mu_);
    for (const auto &ref : returned_refs) {
      const auto obj_id = ObjectID::FromBinary(ref.object_id());
      gossip_table_[obj_id] = task_spec.GetMessage();
      RAY_LOG(INFO).WithField(obj_id)
          << "GOSSIP_STORE: stored task spec for object "
          << obj_id.Hex()
          << " table_size=" << gossip_table_.size();
    }
  }
...
void CoreWorker::HandlePushTask(rpc::PushTaskRequest request,
  ...
  // GOSSIP STORE on executor: store task spec so peers can recover if we die
  // Also send gossip back to caller so they have our metadata
  if (request.task_spec().type() == TaskType::NORMAL_TASK) {
...
void CoreWorker::HandleGossipFuture(
    rpc::GossipFutureRequest request,
    rpc::GossipFutureReply *reply,
    rpc::SendReplyCallback send_reply_callback) {
  if (!gossip_recovery_enabled_) {
    send_reply_callback(Status::OK(), nullptr, nullptr);
    return;
  }
  const auto obj_id = ObjectID::FromBinary(request.object_id());
  RAY_LOG(INFO).WithField(obj_id)
      << "GOSSIP_RECEIVED: received gossip for " << obj_id.Hex();
  {
    absl::MutexLock lock(&gossip_mu_);
    gossip_table_[obj_id] = request.task_spec();
    RAY_LOG(INFO).WithField(obj_id)
        << "GOSSIP_RECEIVED: stored in local table"
        << " table_size=" << gossip_table_.size();
  }
  send_reply_callback(Status::OK(), nullptr, nullptr);
}

void CoreWorker::HandleNumPendingTasks(
    rpc::NumPendingTasksRequest request,
