...
Status CoreWorker::GetObjectsInternal(
  ...
  if (!got_exception && !plasma_object_ids.empty()) {
    ...
    if (timeout_ms >= 0) {
      local_timeout_ms = std::max<int64_t>(
          0,
          timeout_ms -
  ...
  // OWNER_DIED can be returned either by the memory store or by
  // the plasma fetch. Inspect the final result before returning it.
  if (allow_recovery_succession &&
    ...
    if (!owner_died_objects.empty()) {
      ...
      for (const ObjectID &object_id :
           owner_died_objects) {
        auto recovery_promise =
            std::make_shared<std::promise<bool>>();

        std::future<bool> recovery_future =
            recovery_promise->get_future();
  ...
  Status CoreWorker::Contains(
      const ObjectID &object_id, bool *has_object, bool *is_in_plasma) {
    bool found = false;
  ...
  Status CoreWorker::GetLocationFromOwner(
    ...
    auto location_by_id = std::make_shared<
        absl::flat_hash_map<ObjectID, std::shared_ptr<ObjectLocation>>>();
  ...
  Status CoreWorker::CreateActor(const RayFunction &function,
    ...
    SubscribeToNodeChanges();
    RAY_CHECK(
        actor_creation_options.scheduling_strategy.scheduling_strategy_case() !=
        rpc::SchedulingStrategy::SchedulingStrategyCase::SCHEDULING_STRATEGY_NOT_SET);
    ...
    if (actor_name.empty()) {
      io_service_.post(
          [this, task_spec = std::move(task_spec)]() {
            actor_creator_->AsyncRegisterActor(
  ...
  ObjectID CoreWorker::AsyncWaitPlacementGroupReady(
    ...
    gcs_client_->GetGcsRpcClient().WaitPlacementGroupUntilReady(
        ...
        [this, object_id, serialized_object_data, serialized_object_metadata](
          ...
          if (status.ok()) {
            auto data =
                std::make_shared<LocalMemoryBuffer>(serialized_object_data.size());
            memcpy(data->Data(),
                   serialized_object_data.data(),
                   serialized_object_data.size());
            auto metadata =
  ...
  Status CoreWorker::CancelTask(
      const ObjectID &object_id, bool force_kill, bool recursive) {
  ...
  Status CoreWorker::SerializeActorHandle(
      const ActorID &actor_id, std::string *output, ObjectID *actor_handle_id) const {
    auto actor_handle = actor_manager_->GetActorHandle(actor_id);
  ...
  std::shared_ptr<const ActorHandle> CoreWorker::GetActorHandle(const ActorID &actor_id)
      const {
          ...
          [this] {
            ...
            if (status.IsUnexpectedSystemExit()) {
              Exit(rpc::WorkerExitType::SYSTEM_ERROR,
                   absl::StrCat("Worker exits unexpectedly by a signal. ",
                                status.message()),
                   nullptr);
            }
          },
  ...
  Status CoreWorker::ExecuteTask(
    ...
    if (!borrowed_ids.empty()) {
      reference_counter_->PopAndClearLocalBorrowers(
          borrowed_ids, borrowed_refs, &deleted);
    }
  ...
  void CoreWorker::SetAsyncGeneratorBackpressureUnblockNotify(
      const ObjectID &generator_id, void (*fn)(void *), void *ctx) {
      ...
      if (task.ArgByRef(i)) {
        ...
        if (task.ArgDataSize(i) != 0u) {
          data = std::make_shared<LocalMemoryBuffer>(
              const_cast<uint8_t *>(task.ArgData(i)), task.ArgDataSize(i));
        }
    ...
    task_execution_service_.post(
        [this,
         ...
         attempt_number = request.attempt_number()] {
          ...
          actor_task_execution_arg_waiter_->MarkReady(
              TaskAttempt{task_id, attempt_number});
        },
  ...
  void CoreWorker::HandleGetObjectStatus(rpc::GetObjectStatusRequest request,
    ...
    if (HandleWrongRecipient(WorkerID::FromBinary(request.owner_worker_id()),
                             send_reply_callback)) {
      RAY_LOG(INFO)
          << "Handling GetObjectStatus for object produced by a previous worker "
          ...
          PublishRecoveryManifestToWitnesses(
              ...
              [this,
               ...
               send_reply_callback = std::move(send_reply_callback)](
                ...
                if (!manager->CommitHolderAdmission(reservation_id,
                                                    &committed_manifest)) {
                  reply->set_result(rpc::ReportRecoveryCandidateReply::STALE_MANIFEST);
  ...
  void CoreWorker::TryRecoveryHolders(const ObjectID &object_id,
      ...
      LookupRecoveryManifestFromWitnesses(
          ...
          [this, object_id, return_index, manifest, callback = std::move(callback)](
            ...
            if (!witness_manifest.has_value() ||
                CompareRecoveryManifestVersions(witness_manifest.value(), manifest) <=
                    0) {
              callback(false);
              return;
            }
    ...
    const rpc::RecoveryHolder &holder =
        manifest.succession(static_cast<int>(holder_index));
      ...
      actor_creator_->AsyncWaitForActorRegisterFinish(
          actor_id, [this, actor_id, respond = std::move(respond)](const auto &status) {
            if (!status.ok()) {
              ...
              RAY_LOG(DEBUG).WithField(actor_id)
                  << "Received HandleWaitForActorRefDeleted";
  ...
  Status CoreWorker::GetLocalObjectLocations(
    ...
    for (size_t i = 0; i < object_ids.size(); i++) {
      results->emplace_back(
          TryGetLocalObjectLocation(*reference_counter_, object_ids[i]));
    }
    return Status::OK();
  }
  ...
  Status CoreWorker::DeleteImpl(const std::vector<ObjectID> &object_ids,
                                bool local_only) {
  ...
  void CoreWorker::HandleExit(rpc::ExitRequest request,
    ...
    if (!is_idle) {
      ...
      RAY_LOG_EVERY_MS(INFO, 60000)
          << "Worker is not idle: reference counter: "
          << reference_counter_->DebugString() << " # pins in flight: " << pins_in_flight
  ...
  void CoreWorker::HandlePlasmaObjectReady(rpc::PlasmaObjectReadyRequest request,
    ...
    std::vector<std::function<void(void)>> callbacks;
    {
      absl::MutexLock lock(&plasma_mutex_);
      auto it =
          async_plasma_callbacks_.extract(ObjectID::FromBinary(request.object_id()));
      callbacks = it.mapped();
    }
  ...
  std::vector<rpc::Address> CoreWorker::SelectRecoveryWitnesses(const TaskID &task_id)
      const {
    ...
    for (const rpc::GcsNodeInfo &node : nodes_result.value()) {
      ...
      candidates.push_back(WitnessCandidate{
