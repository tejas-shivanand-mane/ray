...
Status CoreWorker::GetObjectsInternal(const std::vector<ObjectID> &ids,
  ...
  if (allow_recovery_succession && recovery_succession_enabled_ &&
    ...
    if (!recoverable_owner_died_objects.empty()) {
      ...
      // Remove an in-memory OWNER_DIED value, if present. If the
      // memory store only contains OBJECT_IN_PLASMA, it remains as
      // the normal redirection marker.
      memory_store_->Delete(recoverable_owner_died_objects);
      ...
      if (!delete_status.ok()) {
        ...
        if (all_recoveries_accepted) {
          ...
          // The stale local error object is gone. Wait for the
          // replayed task to produce the original ObjectID.
          return GetObjectsInternal(ids,
