...
bool ReferenceCounter::AddOrPromoteOwnedObjectForRecovery(
  ...
  // Preserve the holder's existing local reference count,
  // submitted-task references, nesting information, and
  // downstream borrower information.
  ...
  // The acting owner no longer reports itself as borrowing
  // this object from the failed original owner.
  reference.publish_ref_removed = false;

  // The original value was lost. The replay has not created
  // or pinned its replacement yet.
  ...
  RAY_CHECK(
      reconstructable_owned_objects_index_.emplace(object_id, reconstructable_it).second);
