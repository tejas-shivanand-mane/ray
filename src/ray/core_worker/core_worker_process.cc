...
std::shared_ptr<CoreWorker> CoreWorkerProcessImpl::CreateCoreWorker(
  ...
  // Uses GetCoreWorker() to safely access CoreWorker after construction,
  // same pattern as object_lookup and object_recovery_manager above.
  auto gossip_recovery_callback =
      [this](const ObjectID &object_id) -> bool {
        auto core_worker = GetCoreWorker();
        if (!core_worker) {
          return false;
        }
  ...
  // Register gossip callback — send gossip to newly connected workers
  core_worker->core_worker_client_pool_->SetOnNewConnectionCallback(
      [core_worker_weak = std::weak_ptr<CoreWorker>(core_worker)](
          std::shared_ptr<rpc::CoreWorkerClientInterface> client) {
        auto cw = core_worker_weak.lock();
        if (!cw) return;
        if (!cw->gossip_recovery_enabled_) return;
        absl::flat_hash_map<ObjectID, rpc::TaskSpec> table_copy;
        {
          absl::MutexLock lock(&cw->gossip_mu_);
          table_copy = cw->gossip_table_;
        }
        for (const auto &[obj_id, task_spec] : table_copy) {
          ...
          *gossip_req.mutable_task_spec() = task_spec;
          gossip_req.set_owner_worker_id(cw->rpc_address_.worker_id());
          *gossip_req.mutable_owner_address() = cw->rpc_address_;
          client->GossipFuture(
              gossip_req,
              [obj_id](const Status &gossip_status,
                ...
                if (gossip_status.ok()) {
                  RAY_LOG(INFO).WithField(obj_id)
                      << "GOSSIP_SENT: sent to new peer for "
                      << obj_id.Hex();
                }
              });
        }
