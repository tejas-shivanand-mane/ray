...
def callback_type(monitor_name):
                    ...
                    def emit_and_observe(pair):
                        ...
                        if self.trigger is None:
                            ...
                            if target["output_blocks"] >= 2 and target["active_enrolled_tasks"]:
                                self.trigger = {
                                    "stage": index, "operator": op.name,
                                    "output_rows": target["output_rows"],
...
def validate(observation, require_replay):
    ...
    if require_replay:
        ...
        if not trigger:
            raise ValueError("The workload did not reach protected output progress")
