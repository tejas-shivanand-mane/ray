#!/usr/bin/env python3
"""Fixed-R authoritative-witness node failure + concurrent recovery.

W1 is removed before the task owner fails. Two borrowers then race to recover the
same ObjectRef. W2 must become the authoritative coordinator, and exactly one
replay may start.
"""
...
def main() -> None:
    cluster = None
    marker = Path(tempfile.gettempdir()) / f"ray_fixed_r_w1_fail_{uuid.uuid4().hex}.csv"
    ...
    try:
        ...
        for _ in range(MAX_SELECTION_ATTEMPTS):
            token = f"w1-fail-{uuid.uuid4().hex}"
            tokens.append(token)
            ref = ray.get(
        ...
        # Remove W1 while the owner is still alive. This is a real, authoritative
        # node failure, not a transport timeout.
        cluster.remove_node(w1, allow_graceful=False)
        wait_for_node_state(
            ray, node_id_hex(w1), alive=False, timeout_s=30.0
        )
        ...
        grant_lines = wait_for_log(
            logs,
            "Fixed-R recovery claim granted after witness replication attempt 1 coordinator index 1",
            timeout_s=10.0,
        )
        ...
        print("PASS: Fixed-R authoritative witness death + concurrent recovery")
        print(f"  R                         = {R}")
        print(f"  W1 (dead)                 = {node_id_hex(w1)}")
        print(f"  W2 (coordinator)          = {node_id_hex(w2)}")
        print(f"  W3                        = {node_id_hex(w3)}")
