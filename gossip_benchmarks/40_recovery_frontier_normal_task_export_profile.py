#!/usr/bin/env python3
"""Diagnostic 40: pending-vs-finished export through the NORMAL-task path.

Diagnostic 39 used Consumer.hold.remote(), which is an actor task and therefore
never exercises CoreWorker::SubmitTask's deferred Recovery Frontier dispatch path.
This diagnostic is intentionally identical in spirit, but the downstream holder is
a normal stateless Ray task.  It directly measures whether deferred dispatch removes
the all-R Frontier durability barrier from normal-task submission.

The nested ObjectRef is not dereferenced by hold(); it is only inspected for its
ObjectID, so producer execution time is excluded from the downstream work.
"""
...
def run_case(label: str, recovery_mode: str, k: int | None, export_mode: str) -> dict[str, float]:
    ...
    try:
        ...
        # max_retries=0 keeps the downstream holder itself outside recovery scope.
        # The nested producer ObjectRef remains a borrower/export operation.
...
def main() -> None:
    ...
    print("\nNormal-task pending-vs-finished export profile:")
    print(
