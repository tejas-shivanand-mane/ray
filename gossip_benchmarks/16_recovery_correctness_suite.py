\
#!/usr/bin/env python3
"""Benchmark 16: recovery correctness suite.

This file groups three related recovery-succession correctness experiments.

Run NOW (the only blocker before Patch 4B-2):
    python gossip_benchmarks/16_recovery_correctness_suite.py \
        run --cases sequential_failover --trials 3

Run LATER:
    python gossip_benchmarks/16_recovery_correctness_suite.py \
        run --cases concurrent_recovery retry_version_fallback --trials 5

Cases
-----
1. sequential_failover
   Fully form A -> H1 -> H2, then:
       kill A
       requester R1 triggers H1 replay
       kill H1 while that replay is in flight
       requester R2 triggers H2 replay
   Expected: final recovery succeeds, with at least three START markers:
       original + H1 replay + H2 replay.

   R1 and R2 both receive the fully committed A/H1/H2 manifest before any
   failure. This intentionally isolates the basic succession property from
   recovery_attempt/version propagation.

2. concurrent_recovery
   Fully form A -> H1 -> H2, kill A, then two independent requesters call
   ray.get() concurrently.
   Desired property: both requesters resolve, but only ONE recovery replay is
   started. A duplicate replay is reported as a failure.

3. retry_version_fallback
   Fully form A -> H1 -> H2 with max_retries=1, kill A, let one requester
   trigger H1 recovery, then kill H1 and reuse the SAME requester for fallback.
   Desired property: the recovery budget has already been consumed, so H2 must
   NOT start another replay. This case is intended to expose recovery_attempt /
   same-generation manifest handling. It is deferred until after Patch 4B-2.

Each case runs in a fresh Ray Cluster.
"""
...
def start_cluster(args: argparse.Namespace):
    ...
    # Head/GCS survives and is the only witness when the original task is
    # submitted. H1/H2 are added only after the initial manifest exists.
...
def form_two_holders(
    *,
    ...
    """Form committed H1/H2, then give latest metadata to requesters.

    H1 gets the original ref first. Once H1 is committed, H1 forwards its
    locally updated ref to H2. Once H2 is committed, H2 forwards the fully
    committed A/H1/H2 metadata to every requester.
    """
    ...
    # H1 -> H2. H1's local ObjectRef metadata should now contain the committed
    # generation with H1, which lets H2 become rank 2.
    ray.get(h1.forward_to.remote(h2))
    ...
    # H2 now carries the newest committed A/H1/H2 manifest. Forward it directly
    # to requesters so the sequential-failover case does not depend on later
    # recovery_attempt/version propagation.
    for requester in requesters:
        ray.get(h2.forward_to.remote(requester))
...
def run(args: argparse.Namespace) -> None:
    ...
    if cases == ["sequential_failover"]:
        print(
            "\nDecision gate before Patch 4B-2: "
            "sequential_failover should pass all trials."
        )
...
def parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        ...
        help=(
            "Cases to run. Default: sequential_failover. "
            "Use --cases all for the full suite."
        ),
    )
