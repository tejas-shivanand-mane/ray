"""Replication-count sweep: disabled, Fixed-R K=32 and Succession K=32.

R=W varies over 1,2,3 while application fan-out and local topology stay fixed.
All borrowers must consume each object before its pipeline counts as complete.
Three witness-capable nodes are provisioned for every case, including disabled.
"""
...
def configuration(args) -> dict:
    ...
    return {
        "schema_version": 1,
        "variants": VARIANTS,
        "replication_counts": args.replication_counts,
        "payload_bytes": args.payload_bytes, "frontier_k": 32,
        "topology": "head + producer + B fixed borrowers + 3 witness-capable nodes",
        "borrowers": args.borrowers, "witness_nodes": 3,
        "case_timeout_seconds": args.case_timeout_seconds,
        "task_spec_padding_name": args.task_spec_padding.name,
        "task_spec_padding_bytes": args.task_spec_padding.size_bytes,
        "replication_parameter": "R=W",
        ...
        "durability_measurement": "not measured; completion can overlap holder admission",
        ...
        "benchmark_source_sha256": {
            ...
            for name in ("replication_counts.py", "workload.py", "comparison.py",
...
def key(row) -> tuple[int, str, int]:
    return int(row["replication_count"]), str(row["variant"]), int(row["repetition"])


def validate_rows(rows, config):
    counts = config["replication_counts"]
    if (config["schema_version"] != 1 or config["variants"] != VARIANTS
            or config["replication_parameter"] != "R=W" or config["witness_nodes"] != 3
            or config["borrowers"] < 1
            or config["frontier_k"] != 32 or config["profiling_enabled"]
            or config["repetitions"] < 2 or not counts
            or len(set(counts)) != len(counts) or not set(counts) <= {1, 2, 3}):
        raise ValueError("Invalid replication-count configuration")
    seen = set()
    for row in rows:
        case = key(row)
        count, variant, rep = case
        if (case in seen or count not in config["replication_counts"]
                or variant not in VARIANTS or not 1 <= rep <= config["repetitions"]):
            raise ValueError(f"Duplicate or unexpected saved case: {case}")
        seen.add(case)
        expected = {
            "holders": count, "borrowers_per_pipeline": config["borrowers"], "profiling_enabled": 0,
...
def aggregate(rows):
    """Keep R/W values separate; form ratios only in complete paired blocks."""
    summaries, paired = [], []
    for count in sorted({int(row["replication_count"]) for row in rows}):
        subset = [row for row in rows if int(row["replication_count"]) == count]
        for item in b59.summary_rows(subset):
            item["replication_count"] = count
            item["holders"] = count
            item["witness_count"] = count
            item["borrowers_per_pipeline"] = int(subset[0]["borrowers_per_pipeline"])
        ...
        for variant, overheads in samples.items():
            if not overheads:
                continue
            item = {
                "replication_count": count, "holders": count, "witness_count": count,
                "variant": variant,
...
def write_outputs(out, rows):
    ...
    save_csv(out / "replication_count_runs.csv", rows)
    save_csv(out / "replication_count_summary.csv", summaries)
    save_csv(out / "replication_count_paired.csv", paired)
    return summaries, paired


def plot_saved(out):
    config = json.loads((out / "run_config.json").read_text())
    rows = b59.read_csv(out / "replication_count_runs.csv")
    seen = validate_rows(rows, config)
    expected = len(config["replication_counts"]) * len(VARIANTS) * config["repetitions"]
    if len(seen) != expected:
        raise ValueError(f"Only {len(seen)}/{expected} cases complete; resume before plotting")
    summaries, paired = aggregate(rows)
    plot_replication(summaries, paired, out, config)
    ...
    print("\nFinal replication-count comparison (all-borrower application completion):")
    for count in sorted(config["replication_counts"]):
        print(f"\n  R=W={count}")
        for variant in VARIANTS:
...
def run(args):
    ...
    rows = [dict(row) for row in b59.read_csv(out / "replication_count_runs.csv")]
    complete = validate_rows(rows, config)
    variant_order = list(VARIANTS)
    count_order = list(args.replication_counts)
    ...
    print(f"Replication-count sweep: {len(cases)} cases, {len(pending)} remaining; K=32",
          flush=True)
    print("  profiling OFF; fresh cluster/case; fixed burst and in-flight task counts", flush=True)
    print(f"  R=W values: {args.replication_counts}; payload: {args.payload_bytes} bytes", flush=True)
    print(f"  TaskSpec padding fixed at {args.task_spec_padding.size_bytes} bytes", flush=True)
    print(f"  {args.borrowers} borrowers and 3 witness-capable nodes for every case; "
          f"{args.borrowers + 5} local Ray nodes total", flush=True)
    if args.borrowers < max(args.replication_counts):
        print("  B<R for some cases: application throughput does not certify target durability",
              flush=True)
    ...
    for i, (rep, count, variant, position) in enumerate(pending, 1):
        ...
        case_args.holders = case_args.witness_count = count
        command = b59.perf_cmd(case_args, variant, args.task_spec_padding, rep, temporary)
        command[1] = str(Path(__file__).resolve())
        command += ["--single-borrowers", str(args.borrowers)]
        print(f"[{i}/{len(pending)}] rep={rep} R=W={count} "
...
def parser():
    ...
    p.add_argument("--rw-values", dest="replication_counts", type=int, nargs="+",
                   choices=(1, 2, 3), default=[1, 2, 3], help="Equal R and W values")
    p.add_argument("--borrowers", type=int, default=3,
                   help="Fixed application fan-out for every R/W value")
    ...
    p.add_argument("--output-dir", type=Path, default=HERE.parent / "results" / "replication_counts")
...
def main():
    ...
    # R/W explicitly. Existing benchmarks retain their R=2/W=2 defaults.
    if len(sys.argv) > 1 and sys.argv[1] == "_single-perf":
        ...
        if args.holders not in (1, 2, 3) or args.witness_count != args.holders:
            p.error("Child requires R=W in 1,2,3")
        row = b59.b58.single_perf(
            args, borrower_count=args.single_borrowers,
            replication_sweep=True, witness_nodes=3)
        row["method"] = b59.method_family(args.single_variant)
        row["witness_count"] = args.witness_count
        row["replication_count"] = args.holders
        row["witness_nodes"] = 3
    ...
    if len(set(args.replication_counts)) != len(args.replication_counts):
        p.error("--rw-values must not contain duplicates")
    if args.borrowers < 1:
        p.error("--borrowers must be positive")
