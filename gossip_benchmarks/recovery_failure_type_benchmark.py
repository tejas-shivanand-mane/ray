#!/usr/bin/env python3
"""
Prototype benchmark: worker failure vs node failure.

Three failure modes are supported:
  1. owner_worker:
...
This separates ownership loss from full compute/object loss and helps determine
whether recovery cost is dominated by Ray's worker-failure or node-failure path.

The benchmark uses a persistent borrower and explicit recovery holders.
"""
...
def make_remote_types():
    ...
    def produce(seed: int, duration_s: float, payload_bytes: int, marker_path: str):
    ...
    class Owner:
        ...
        def crash(self):
            # Hard worker-process failure, not graceful actor termination.
            os._exit(66)
...
def form_succession(ref, holders, session_dirs, timeout_s):
    fresh = ref
...
def run_trial(mode: str, args, trial: int):
    row = {
        "trial": trial,
        "failure_mode": mode,
    ...
    marker = Path(tempfile.gettempdir()) / f"ray_failure_type_{os.getpid()}_{uuid.uuid4().hex}.csv"
    layout = None
    ...
    try:
        ...
        expected = 1 + 1 + producer_extra + args.holders + 1 + args.witness_count
        ...
        ref = ray.get(
            owner.dispatch.remote(seed, args.task_duration, args.payload_bytes, str(marker))
        )[0]

        first_start = wait_for_first_start(marker, args.start_timeout)

        ref = form_succession(ref, holders, session_dirs, args.formation_timeout)
        ...
        if mode == "owner_worker":
            try:
                ray.get(owner.crash.remote(), timeout=5)
            except Exception:
                pass
        elif mode == "owner_node":
            layout.cluster.remove_node(layout.owner_node, allow_graceful=False)
        elif mode == "owner_plus_producer_node":
            layout.cluster.remove_node(layout.owner_node, allow_graceful=False)
        else:
        ...
        try:
            ...
            row["failure_to_result_s"] = time.perf_counter() - failure_perf
        ...
        second_start = wait_for_second_start(marker, timeout_s=1.0)
        if second_start is not None:
            row["failure_to_replay_start_s"] = max(
                0.0, (second_start[1] - failure_wall_ns) / 1e9
            )
        ...
        # In owner_worker/owner_node modes the original producer node survives.
        # A FINISH from the original producer PID after failure is evidence that
        # ownership loss can coexist with the original compute still completing.
        if mode != "owner_plus_producer_node" and finishes:
            ...
            row["original_task_finished_after_owner_failure"] = any(
                e[2] == original_pid and e[1] > failure_wall_ns for e in finishes
            )
...
def main():
    ...
    parser.add_argument("--output", type=Path, default=Path("failure_type_results.csv"))
    parser.add_argument(
        "--modes",
        nargs="+",
    ...
    for mode in args.modes:
        for trial in range(1, args.trials + 1):
            run_no += 1
            print(f"\n[{run_no}/{total}] mode={mode} trial={trial}")
            row = run_trial(mode, args, trial)
            rows.append(row)
            print(
                f"  success={row['success']} executions={row['executions_observed']} "
